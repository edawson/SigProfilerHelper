...
task GenerateSBSMatrix{
    ...
    command{
        python /data/dawsonet/sandbox/sigprofilerhelper/generate_matrix.py -m ${maf} -d sigprof_input  && \
...
task GenerateID83Matrix{
    ...
    command{
        python /data/dawsonet/sandbox/presig/presig/presig.py -m ${maf} -f ${refFA} && \
...
task tidySigPlot{
    input{
        ...
        command{
            tar xvzf ${sigTGZ} && \
                Rscript /data/dawsonet/sandbox/tidysig/scripts/make_${type}_plots.R -i ${base} -o ${base}_${type}_plots && \
